        ...
        if content_type == 'text':
            ...
            if msg_text == '/start':
                BotOutput.send_plain_text(
                    bot, user, "Hello 我是 Foodge( º﹃º )，\n輸入 /help 來瞭解怎麼操(›´ω`‹ )\n作我喔 <3")
            elif msg_text == '/help':
                BotOutput.send_plain_text(
                    bot, user, "(✺ω✺)foodge提供的服務有：\n\n /set - 更改設定\n /advise - 利用問答題幫你挑食物\n /random - 直接幫你選一種食物\n /tourist - 告訴你附近有什麼美食\n /locate - 告訴你它在哪裡")
            elif msg_text == '/set':
                BotOutput.send_plain_text(
                ...
                BotOutput.send_location_request(bot, user, "( ͡ʘ ͜ʖ ͡ʘ)請先將你的位置發給我喔～")
                ...
                BotOutput.send_plain_text(bot, user, "(๑•́ ₃ •̀๑)我來隨便找找看 ㄏㄏ")
                while len(user.restaurants) < 1:
                    food_name = DatabaseHelper.get_rand_food()
                ...
                BotOutput.send_plain_text(bot, user, "(´c_`)告訴我一個你想找的食物名稱？")
                user.next_status = '輸入食物名稱'
            else:
                BotOutput.send_plain_text(bot, user, "(´ﾟдﾟ`)輸入 /help 獲得幫助")
        else:
            BotOutput.send_plain_text(bot, user, "輸入 /help 獲得幫助ΩДΩ")
